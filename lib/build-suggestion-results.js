...
const LIKELY_LANE_MATCHUP_WEIGHT = 2;
...
/**
 * Give every independently inferred same-role enemy a full second
 * contribution. This intentionally does not assign one unique lane opponent:
 * if three enemies are each likely Top, all three receive the same 2x weight
 * for a Top build.
 */
function expandMatchupBuildsByLaneWeight(matchupBuilds) {
  const weightedMatchupBuilds = [];

  for (const matchup of Array.isArray(matchupBuilds) ? matchupBuilds : []) {
    const allyRole = normalizeRole(matchup?.role);
    const enemyRole = normalizeRole(matchup?.enemyRole);
    const weight = allyRole && enemyRole === allyRole ? LIKELY_LANE_MATCHUP_WEIGHT : 1;
