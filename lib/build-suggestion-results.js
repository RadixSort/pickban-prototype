...
const LIKELY_LANE_MATCHUP_WEIGHT = 3;
...
function buildBuildSuggestionResults({
  ...
  const weightedMatchupBuilds = expandMatchupBuildsByLaneWeight(matchupBuilds);
...
/**
 * Give every independently inferred same-role enemy three contributions. If
 * role inference produces no same-lane enemy, still triple the matchup with
 * the highest explicit lane likelihood (or largest matchup sample as a stable
 * fallback). This guarantees that at least one likely lane opponent is
 * represented more heavily.
 */
function expandMatchupBuildsByLaneWeight(matchupBuilds) {
  const normalizedMatchupBuilds = Array.isArray(matchupBuilds) ? matchupBuilds : [];
  const likelyLaneOpponentIndexes = findLikelyLaneOpponentIndexes(normalizedMatchupBuilds);
  const weightedMatchupBuilds = [];

  normalizedMatchupBuilds.forEach((matchup, index) => {
    const weight = likelyLaneOpponentIndexes.has(index) ? LIKELY_LANE_MATCHUP_WEIGHT : 1;

    for (let contribution = 0; contribution < weight; contribution += 1) {
      weightedMatchupBuilds.push(matchup);
    }
  });

  return weightedMatchupBuilds;
}

function findLikelyLaneOpponentIndexes(matchupBuilds) {
  const inferredSameLaneIndexes = new Set();

  matchupBuilds.forEach((matchup, index) => {
    const allyRole = normalizeRole(matchup?.role);
    const enemyRole = normalizeRole(matchup?.enemyRole);
    if (allyRole && enemyRole === allyRole) {
      inferredSameLaneIndexes.add(index);
    }
  });

  if (inferredSameLaneIndexes.size > 0 || matchupBuilds.length === 0) {
    return inferredSameLaneIndexes;
  }

  let mostLikelyIndex = 0;
  for (let index = 1; index < matchupBuilds.length; index += 1) {
    if (compareLaneOpponentLikelihood(matchupBuilds[index], matchupBuilds[mostLikelyIndex]) < 0) {
...
function compareLaneOpponentLikelihood(left, right) {
  return (
    toNumber(right?.laneOpponentLikelihood) - toNumber(left?.laneOpponentLikelihood) ||
    toNumber(right?.totalGames) - toNumber(left?.totalGames) ||
    String(left?.enemyChampionKey || "").localeCompare(String(right?.enemyChampionKey || ""))
  );
}
