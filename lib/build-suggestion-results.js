...
function buildStartingItemResults(startingItemMap, thresholdPct) {
  const itemOptions = [...startingItemMap.values()];
  let totalItemGames = 0;

  for (const option of itemOptions) {
    totalItemGames += toNumber(option.games);
  }

  const mostPickedSet = selectBestRecord(itemOptions, compareMostPickedSets);
  const highestWinSet = selectBestRecord(
    itemOptions,
    compareHighestWinSets,
    (option) => calculateRate(option.games, totalItemGames) >= thresholdPct,
  );
  const highlightingNotes = [];

  if (!highestWinSet && itemOptions.length > 0) {
    highlightingNotes.push(
  ...
  return {
    options: itemOptions
      .map((option) =>
        hydrateStartingItemSetResult(option, totalItemGames, mostPickedSet, highestWinSet),
      )
      .sort(compareHighestWinSets),
    mostPickedSet: hydrateStartingItemSetResult(
      mostPickedSet,
      totalItemGames,
      mostPickedSet,
      highestWinSet,
    ),
    highestWinSet: hydrateStartingItemSetResult(
      highestWinSet,
      totalItemGames,
      mostPickedSet,
      highestWinSet,
...
function hydrateStartingItemSetResult(itemRecord, totalItemGames, mostPickedSet, highestWinSet) {
  if (!itemRecord) {
    return null;
  }

  return {
    setKey: itemRecord.setKey,
    itemIds: itemRecord.itemIds,
    selections: itemRecord.selections,
    games: Math.round(toNumber(itemRecord.games)),
    wins: Math.round(toNumber(itemRecord.wins)),
    winRate: calculateRate(itemRecord.wins, itemRecord.games),
    pickRate: calculateRate(itemRecord.games, totalItemGames),
    isMostPicked: mostPickedSet?.setKey === itemRecord.setKey,
    isHighestWin: highestWinSet?.setKey === itemRecord.setKey,
  };
}
...
function mergeStartingItemOptions(targetMap, startingItemOptions) {
  if (!Array.isArray(startingItemOptions)) {
    return;
  }

  startingItemOptions.forEach((itemOption) => {
    if (!itemOption?.setKey) {
      return;
    }

    const existing = targetMap.get(itemOption.setKey);
    if (existing) {
      existing.games += toNumber(itemOption.games);
      existing.wins += toNumber(itemOption.wins);
      return;
    }

    targetMap.set(itemOption.setKey, {
      ...itemOption,
      itemIds: Array.isArray(itemOption.itemIds) ? [...itemOption.itemIds] : [],
      selections: Array.isArray(itemOption.selections) ? [...itemOption.selections] : [],
      games: toNumber(itemOption.games),
      wins: toNumber(itemOption.wins),
    });
  });
}
