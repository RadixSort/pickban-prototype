...
function buildBuildSuggestionResults({
  ...
  const laneOpponentMatchupBuilds = matchupBuilds.filter((_matchup, index) =>
    laneOpponentIndexes.has(index),
  );
  const totalGames = matchupBuilds.reduce(
  ...
  const runes = buildRuneResultsFromMatchups(
    matchupBuilds,
    highestWinPageThresholdPct,
  );
  const laneOpponentRunes = buildRuneResultsFromMatchups(
    laneOpponentMatchupBuilds,
    highestWinPageThresholdPct,
  );
  ...
  const items = {
    ...buildItemResultsFromMatchups(
      matchupBuilds,
      highestWinItemThresholdPct,
    ),
    laneOpponents: buildItemResultsFromMatchups(
      laneOpponentMatchupBuilds,
      highestWinItemThresholdPct,
    ),
  };
  ...
  return {
    ...
    runes: {
      ...runes,
      laneOpponents: laneOpponentRunes,
    },
    spells,
    startingItems,
    skillPriority,
...
function buildRuneResultsFromMatchups(matchupBuilds, highestWinPageThresholdPct) {
  const totalGames = matchupBuilds.reduce(
    (sum, matchup) => sum + toNumber(matchup?.totalGames),
    0,
  );
...
function buildItemResultsFromMatchups(matchupBuilds, highestWinItemThresholdPct) {
  ...
  matchupBuilds.forEach((matchup) => {
    ...
    mergeGroupedItemOptionLists(
      mostPickedItemSlotMaps,
      matchup?.items?.mostPickedSlotOptions,
    );
    mergeGroupedItemOptionLists(
      highestWinItemSlotMaps,
      matchup?.items?.highestWinSlotOptions,
    );
  });
...
function createPreferredItemSlotGroups(preferredSlotMaps, fallbackSlotGroups) {
  ...
  return Array.from({ length: ITEM_SLOT_COUNT }, (_, index) => {
    ...
    const preferredGroup = preferredGroupsBySlot.get(slotIndex) || null;
    const fallbackGroup = fallbackGroupsBySlot.get(slotIndex) || null;
    if (!preferredGroup) {
    ...
    return {
      ...preferredGroup,
      fallbackOptions: fallbackGroup?.options || [],
    };
  }).filter(Boolean);
}
...
function buildOrderedItemBuild(slotGroups, comparator, thresholdPct = 0) {
  ...
  const candidatesByRow = selectedRows.map(({ slotGroup }) =>
    rankItemSlotOptions(slotGroup, comparator, thresholdPct).filter(
      (option) =>
        !isCompletedBootItem(option.itemId, option.name),
    ),
  );
  const itemPairs = assignUniqueItemPairs(candidatesByRow);
  const selections = selectedRows.map(({ selection, slotGroup }, rowIndex) => {
    const [primary = selection, alternative = null] = itemPairs[rowIndex] || [];
...
/**
 * Match up to two globally distinct items per row. The first pass preserves
 * one recommendation for every selected row; the second uses augmenting paths
 * to maximize complete pairs. Returned choices follow each row's ranking so A
 * is always the stronger of its assigned options.
 */
function assignUniqueItemPairs(candidateRows) {
  const candidatesByRow = Array.isArray(candidateRows) ? candidateRows : [];
  const rowCount = candidatesByRow.length;
  const assignments = Array.from({ length: rowCount * 2 }, () => null);
  const assignedDemandByItemId = new Map();

  function tryAssign(demandIndex, visitedItemIds) {
    const rowIndex = demandIndex % rowCount;
    const candidates = Array.isArray(candidatesByRow[rowIndex])
      ? candidatesByRow[rowIndex]
      : [];

    for (const candidate of candidates) {
      const itemId = candidate?.itemId;
      if (!itemId || visitedItemIds.has(itemId)) {
        continue;
      }
      visitedItemIds.add(itemId);

      const assignedDemandIndex = assignedDemandByItemId.get(itemId);
      if (
        assignedDemandIndex == null ||
        tryAssign(assignedDemandIndex, visitedItemIds)
      ) {
        assignedDemandByItemId.set(itemId, demandIndex);
        assignments[demandIndex] = candidate;
        return true;
      }
    }
  ...
  for (let rowIndex = rowCount - 1; rowIndex >= 0; rowIndex -= 1) {
    tryAssign(rowIndex, new Set());
  }

  for (let rowIndex = 0; rowIndex < rowCount; rowIndex += 1) {
    if (assignments[rowIndex]) {
      tryAssign(rowCount + rowIndex, new Set());
    }
  }

  return candidatesByRow.map((candidates, rowIndex) => {
    const assignedItemIds = new Set(
      [assignments[rowIndex], assignments[rowCount + rowIndex]]
        .filter(Boolean)
        .map((option) => option.itemId),
    );
    return candidates.filter((option) => assignedItemIds.has(option.itemId)).slice(0, 2);
  });
}

function rankItemSlotOptions(slotGroup, comparator, thresholdPct) {
  const rankedPreferredOptions = rankItemOptions(
    slotGroup?.options,
    comparator,
    thresholdPct,
  );
  const rankedFallbackOptions = rankItemOptions(
    slotGroup?.fallbackOptions,
    comparator,
    thresholdPct,
  );
  const seenItemIds = new Set();

  return [...rankedPreferredOptions, ...rankedFallbackOptions].filter((option) => {
    if (seenItemIds.has(option.itemId)) {
      return false;
    }
    seenItemIds.add(option.itemId);
    return true;
  });
}
