...
const legendaryItemIdsCacheByRequester = new WeakMap();
...
async function fetchLiveGameImport({
  ...
  const queueId = extractQueueId(gameflowSession);
  const supportedQueue = SUPPORTED_QUEUE_BY_ID.get(queueId);
  let legendaryItemIds = null;

  try {
    legendaryItemIds = await getLegendaryItemIdsForConnection({
      credentials,
      requestJson,
      timeoutMs,
    });
  } catch (_error) {
    // Champion, lane, and build-gold updates remain useful when the patch item
    // catalog is temporarily unavailable. A null first-item state disables the
    // automatic subset until Legendary classification recovers.
  }
  ...
  try {
    const snapshot = await fetchLiveSnapshot({
      ...
      championBySlug,
      env,
      legendaryItemIds: legendaryItemIds || new Set(),
      normalizeRole,
      timeoutMs,
    });
    ...
    return buildLiveGameImportPayload({
      ...
      snapshot,
      supportedQueue,
      firstItemStatusKnown: legendaryItemIds instanceof Set,
    });
  } catch (_error) {
    return buildLiveGameTransitionPayload({
      gameflowSession,
...
async function getLegendaryItemIdsForConnection({
  credentials,
  requestJson,
  timeoutMs = LEAGUE_CLIENT_TIMEOUT_MS,
} = {}) {
  let cacheByConnection = legendaryItemIdsCacheByRequester.get(requestJson);
  if (!cacheByConnection) {
    cacheByConnection = new Map();
    legendaryItemIdsCacheByRequester.set(requestJson, cacheByConnection);
  }

  const connectionKey = buildLeagueClientConnectionKey(credentials);
  let itemIdsPromise = cacheByConnection.get(connectionKey);
  if (!itemIdsPromise) {
    itemIdsPromise = requestJson(credentials, LEGENDARY_ITEM_CATALOG_PATH, {
      timeoutMs,
    }).then((itemCatalog) => {
      const legendaryItemIds = buildLegendaryItemIdSet(itemCatalog);
      if (legendaryItemIds.size === 0) {
        throw new Error("The League Client item catalog did not contain Legendary items.");
      }
      return legendaryItemIds;
    });
    cacheByConnection.set(connectionKey, itemIdsPromise);
  }

  try {
    return await itemIdsPromise;
  } catch (error) {
    if (cacheByConnection.get(connectionKey) === itemIdsPromise) {
...
function buildLiveGameImportPayload({
  ...
  const allies = snapshot.allies.map((player) =>
    buildPublicLiveGameParticipant(player, { firstItemStatusKnown }),
  );
...
function buildPublicLiveGameParticipant(player, { firstItemStatusKnown = true } = {}) {
  return {
    ...
    buildGold: Number(player?.buildGold || 0),
    buildGoldRank: Number(player?.buildGoldRank || 0),
