...
const CHAMP_SELECT_PHASE_BAN = "ban";
const CHAMP_SELECT_PHASE_PICK = "pick";
const LIVE_GAME_CHAMP_SELECT_PHASE = "in_game";
const LEGENDARY_ITEM_CATALOG_PATH = "/lol-game-data/assets/v1/items.json";
const LIVE_GAMEFLOW_PHASES = new Set(["GameStart", "InProgress", "Reconnect"]);
const itemCatalogMetricsCacheByRequester = new WeakMap();

const SUPPORTED_QUEUE_BY_ID = new Map([
  [0, { description: "Practice Tool", type: "practice" }],
...
async function fetchLiveDraftImport({
  ...
  championBySlug,
  env = process.env,
  fetchLiveSnapshot = fetchLiveGameSnapshot,
  normalizeRole,
  ...
  if (!lockfilePath) {
    return buildDisabledLiveDraftPayload(
      "lockfile_not_found",
  ...
  let gameflowSession;
  try {
    gameflowSession = await requestJson(credentials, "/lol-gameflow/v1/session", {
      timeoutMs,
    });
  } catch (_error) {
    return buildDisabledLiveDraftPayload(
  ...
  if (statusOnly) {
    return buildLiveDraftStatusPayload(gameflowSession);
  }

  if (isLiveGameflowPhase(gameflowPhase)) {
    return fetchLiveGameImport({
      championByName,
      championBySlug,
      credentials,
      env,
  ...
  try {
    ...
    return buildDisabledLiveDraftPayload(reason, message);
  }
}

function buildLiveDraftStatusPayload(gameflowSession) {
  const gameflowPhase = normalizeGameflowPhase(gameflowSession?.phase);
  ...
  return {
    status: "active",
    active: true,
    source: isLiveGameflowPhase(gameflowPhase) ? "live_game" : "champ_select",
    reason: "",
    message: "",
    gameflowPhase,
    champSelectPhase:
      gameflowPhase === "ChampSelect" ? "unknown" : LIVE_GAME_CHAMP_SELECT_PHASE,
    sessionId: extractChampSelectSessionId(null, gameflowSession),
...
function buildLiveDraftImport({
  ...
  if (!supportedQueue) {
    return buildDisabledLiveDraftPayload(
      "unsupported_queue",
      "Auto champion import is disabled: this mode is not Practice Tool, Normal Draft, or Ranked.",
      {
        gameflowPhase,
  ...
  const champSelectPhase = detectChampSelectPhase(champSelectSession);
  const sessionId = extractChampSelectSessionId(champSelectSession, gameflowSession);
  ...
  const bannedChampionKeys = collectBannedChampionKeys(champSelectSession);
  const allyHovers = normalizeAllyHoverIntents(champSelectSession, {
    bannedChampionKeys,
    championByKey,
    normalizeRole,
  });
  ...
  const unavailableChampionKeys = collectUnavailableChampionKeys(champSelectSession, {
    bannedChampionKeys,
    championByKey,
    selectedPlayers: [...lockedAllies, ...enemies],
  });
...
function buildInactiveLiveDraftPayload(gameflowSession) {
  const gameflowPhase = normalizeGameflowPhase(gameflowSession?.phase);
  if (gameflowPhase !== "ChampSelect" && !isLiveGameflowPhase(gameflowPhase)) {
    return buildDisabledLiveDraftPayload(
      "not_in_champ_select",
      "Auto champion import is disabled: no active pick/ban phase was found.",
      { gameflowPhase },
...
async function fetchLiveGameImport({
  championByName,
  championBySlug,
  credentials,
  env,
  ...
  const queueId = extractQueueId(gameflowSession);
  const supportedQueue = SUPPORTED_QUEUE_BY_ID.get(queueId);
  let itemCatalogMetrics = null;

  try {
    itemCatalogMetrics = await getItemCatalogMetricsForConnection({
      credentials,
      requestJson,
      timeoutMs,
    });
  } catch (_error) {
    // Champion and lane updates remain useful when the patch item catalog is
    // temporarily unavailable. Inventory metrics stay unknown until full item
    // costs and Legendary classifications can be resolved accurately.
  }
  ...
  try {
    const snapshot = await fetchLiveSnapshot({
      ...
      championBySlug,
      env,
      itemCostById: itemCatalogMetrics?.itemCostById || new Map(),
      legendaryItemIds: itemCatalogMetrics?.legendaryItemIds || new Set(),
      normalizeRole,
      timeoutMs,
    });

    if (!snapshot?.active) {
      return buildLiveGameTransitionPayload({
        gameflowSession,
        queueId,
        supportedQueue,
      });
    ...
    return buildLiveGameImportPayload({
      gameflowPhase,
      gameflowSession,
      queueId,
      snapshot,
      supportedQueue,
      firstItemStatusKnown:
        itemCatalogMetrics?.legendaryItemIds instanceof Set &&
        itemCatalogMetrics.legendaryItemIds.size > 0,
    });
  } catch (_error) {
    return buildLiveGameTransitionPayload({
      gameflowSession,
...
async function getItemCatalogMetricsForConnection({
  credentials,
  requestJson,
  timeoutMs = LEAGUE_CLIENT_TIMEOUT_MS,
} = {}) {
  let cacheByConnection = itemCatalogMetricsCacheByRequester.get(requestJson);
  if (!cacheByConnection) {
    cacheByConnection = new Map();
    itemCatalogMetricsCacheByRequester.set(requestJson, cacheByConnection);
  }

  const connectionKey = buildLeagueClientConnectionKey(credentials);
  let metricsPromise = cacheByConnection.get(connectionKey);
  if (!metricsPromise) {
    metricsPromise = requestJson(credentials, LEGENDARY_ITEM_CATALOG_PATH, {
      timeoutMs,
    }).then((itemCatalog) => {
      const itemCostById = buildItemCostById(itemCatalog);
      const legendaryItemIds = buildLegendaryItemIdSet(itemCatalog);
      if (itemCostById.size === 0) {
        throw new Error("The League Client item catalog did not contain usable item costs.");
      }
      return {
        itemCostById,
        legendaryItemIds,
    ...
    cacheByConnection.set(connectionKey, metricsPromise);
  }

  try {
    return await metricsPromise;
  } catch (error) {
    if (cacheByConnection.get(connectionKey) === metricsPromise) {
      cacheByConnection.delete(connectionKey);
    }
    throw error;
  }
}

function buildLeagueClientConnectionKey(credentials = {}) {
  return [credentials.protocol, credentials.port, credentials.password].join(":");
}

function buildLiveGameImportPayload({
  ...
  const localPlayer = snapshot.allies.find((player) => player.isLocalPlayer) || null;
  const fetchedAt = snapshot.fetchedAt || new Date().toISOString();
  const allies = snapshot.allies.map((player) =>
    buildPublicLiveGameParticipant(player, {
      buildGoldRanksKnown: snapshot.metricsComplete === true,
      firstItemStatusKnown,
    }),
  );
  const enemies = snapshot.enemies.map((player) =>
    buildPublicLiveGameParticipant(player, {
      buildGoldRanksKnown: snapshot.metricsComplete === true,
      firstItemStatusKnown,
    }),
  ...
  return {
    ...
    source: "live_game",
    reason: "",
    message: "Live game champions, lanes, items, and build gold ranks are automatically being updated.",
    gameflowPhase,
    champSelectPhase: LIVE_GAME_CHAMP_SELECT_PHASE,
    sessionId: extractChampSelectSessionId(null, gameflowSession),
    queue: buildQueueMetadata(queueId, supportedQueue),
    assignedRole: localPlayer?.role || null,
    localPlayerCellId: null,
    localPlayerChampionKey: localPlayer?.championKey || "",
    allies,
    allyHovers: [],
    unavailableChampionKeys: [],
    enemies,
    liveGame: buildLiveGameMetadata(snapshot, fetchedAt, {
      firstItemStatusKnown,
      queueId,
    }),
    fetchedAt,
...
function buildPublicLiveGameParticipant(
  player,
  { buildGoldRanksKnown = true, firstItemStatusKnown = true } = {},
) {
  const inventoryKnown = player?.inventoryKnown === true;
  const completedLegendaryItemCount = normalizeCompletedLegendaryItemCount(
    player?.completedLegendaryItemCount,
    player?.hasCompletedFirstItem,
  );
  return {
    champion: typeof player?.champion === "string" ? player.champion : "",
    championKey: player?.championKey == null ? "" : String(player.championKey),
    championId: typeof player?.championId === "string" ? player.championId : "",
    icon: typeof player?.icon === "string" ? player.icon : "",
    role: player?.role || null,
    isLocalPlayer: player?.isLocalPlayer === true,
    buildGold: inventoryKnown ? Number(player?.buildGold || 0) : null,
    buildGoldRank:
      inventoryKnown && buildGoldRanksKnown ? Number(player?.buildGoldRank || 0) : null,
    inventoryKnown,
    completedLegendaryItemCount:
      firstItemStatusKnown ? completedLegendaryItemCount : null,
    hasCompletedFirstItem:
      firstItemStatusKnown && completedLegendaryItemCount != null
        ? completedLegendaryItemCount > 0
        : firstItemStatusKnown && typeof player?.hasCompletedFirstItem === "boolean"
          ? player.hasCompletedFirstItem
          : null,
  };
}

function buildLiveGameTransitionPayload({
  gameflowSession,
  queueId = extractQueueId(gameflowSession),
  supportedQueue = SUPPORTED_QUEUE_BY_ID.get(queueId),
} = {}) {
  const fetchedAt = new Date().toISOString();
  return {
    status: "active",
    active: true,
    source: "transition",
    reason: "live_game_loading",
    message: "The game has started. Waiting for Live Client Data before updating champions and builds.",
    gameflowPhase: normalizeGameflowPhase(gameflowSession?.phase),
    champSelectPhase: LIVE_GAME_CHAMP_SELECT_PHASE,
    sessionId: extractChampSelectSessionId(null, gameflowSession),
    queue: buildQueueMetadata(queueId, supportedQueue),
    assignedRole: null,
    localPlayerCellId: null,
    localPlayerChampionKey: "",
    allies: [],
    allyHovers: [],
    unavailableChampionKeys: [],
    enemies: [],
    liveGame: buildLiveGameMetadata(null, fetchedAt, {
      firstItemStatusKnown: false,
      queueId,
    }),
    fetchedAt,
  };
...
function buildLiveGameMetadata(
  snapshot,
  fetchedAt,
  { firstItemStatusKnown = false, queueId = null } = {},
) {
  const totalPlayerCount = Number(snapshot?.totalPlayerCount || 0);
  const resolvedPlayerCount = Number(snapshot?.resolvedPlayerCount || 0);
  const omittedParticipantCount = Number(snapshot?.omittedParticipantCount || 0);
  return {
    complete: isCompleteLiveGameSnapshot(snapshot, queueId),
    metricsComplete: snapshot?.metricsComplete === true,
    firstItemStatusKnown: firstItemStatusKnown === true,
    playerCount: totalPlayerCount,
    totalPlayerCount,
    resolvedPlayerCount,
    omittedParticipantCount,
    gameTimeSeconds: normalizeNonnegativeFiniteNumber(snapshot?.gameTimeSeconds),
    fetchedAt,
  };
}
...
function isCompleteLiveGameSnapshot(snapshot, queueId) {
  ...
  return (
    Number(snapshot?.totalPlayerCount) === 10 &&
    Array.isArray(snapshot?.allies) &&
    snapshot.allies.length === 5 &&
    Array.isArray(snapshot?.enemies) &&
    snapshot.enemies.length === 5
  );
}

function isLiveGameflowPhase(value) {
  return LIVE_GAMEFLOW_PHASES.has(normalizeGameflowPhase(value));
}
...
function normalizeAllyHoverIntents(
  champSelectSession,
  {
    bannedChampionKeys = new Set(),
    championByKey,
  ...
  const team = Array.isArray(champSelectSession?.myTeam) ? champSelectSession.myTeam : [];
  const pendingPickChampionByCellId = new Map();
  ...
  for (const action of flattenChampSelectActions(champSelectSession?.actions)) {
    ...
    const actorCellId = normalizeCellId(action.actorCellId);
    const championKey = normalizeChampionKey(action.championId);
    if (actorCellId != null && championKey) {
      pendingPickChampionByCellId.set(actorCellId, championKey);
    }
  }
  ...
  for (const player of team) {
    const cellId = normalizeCellId(player?.cellId);
    const role = normalizeLiveRole(player?.assignedPosition, normalizeRole);
    const championKey =
      normalizeChampionKey(player?.championPickIntent) ||
      pendingPickChampionByCellId.get(cellId) ||
      "";
    const champion = championKey ? championByKey?.get(championKey) || null : null;
    ...
    allyHovers.push({
      ...
      icon: champion.icon,
      role,
      isLocalPlayer: cellId === normalizeCellId(champSelectSession?.localPlayerCellId),
    });
  }
...
function detectChampSelectPhase(champSelectSession) {
  const timerPhase = normalizePhaseToken(champSelectSession?.timer?.phase);
  if (timerPhase.includes("planning")) {
    return "planning";
  }

  if (timerPhase.includes("final") || timerPhase.includes("game_start")) {
    return "finalization";
  }

  const activeActionType = getCurrentChampSelectActionType(
    champSelectSession?.actions,
  ...
  if (champSelectSession?.isInBanPhase === true) {
    return CHAMP_SELECT_PHASE_BAN;
  }

  if (timerPhase.includes("ban") && !timerPhase.includes("pick")) {
    return CHAMP_SELECT_PHASE_BAN;
  }

  if (timerPhase.includes("pick") && !timerPhase.includes("ban")) {
    return CHAMP_SELECT_PHASE_PICK;
  }

  const pendingActionType = getCurrentChampSelectActionType(
    champSelectSession?.actions,
...
function getCurrentChampSelectActionType(actions, predicate) {
  if (!Array.isArray(actions)) {
    return "";
  }

  for (const actionGroup of actions) {
    const group = Array.isArray(actionGroup) ? actionGroup : [actionGroup];
    const currentActions = group.filter(predicate);
    if (currentActions.length === 0) {
      continue;
    }

    const actionTypes = currentActions.map(getChampSelectActionType).filter(Boolean);
    if (actionTypes.includes(CHAMP_SELECT_PHASE_BAN)) {
      return CHAMP_SELECT_PHASE_BAN;
    }

    if (actionTypes.includes(CHAMP_SELECT_PHASE_PICK)) {
      return CHAMP_SELECT_PHASE_PICK;
    }
  }
...
function getChampSelectActionType(action) {
  const type = normalizePhaseToken(action?.type);
  if (type === CHAMP_SELECT_PHASE_BAN || type === CHAMP_SELECT_PHASE_PICK) {
    return type;
  }
...
function collectUnavailableChampionKeys(
  champSelectSession,
  {
    bannedChampionKeys = new Set(),
  ...
  const unavailableChampionKeys = new Set(bannedChampionKeys);

  for (const player of Array.isArray(selectedPlayers) ? selectedPlayers : []) {
    const championKey = normalizeChampionKey(player?.championKey);
    if (championKey) {
      unavailableChampionKeys.add(championKey);
    }
  }

  for (const player of Array.isArray(champSelectSession?.myTeam)
    ? champSelectSession.myTeam
    : []) {
    const championKey = normalizeChampionKey(player?.championPickIntent);
    if (championKey) {
      unavailableChampionKeys.add(championKey);
    }
  }
  ...
  return Array.from(unavailableChampionKeys)
    .filter((championKey) => championByKey.has(championKey))
...
function extractChampSelectSessionId(champSelectSession, gameflowSession) {
  const candidates = [
    champSelectSession?.gameId,
    champSelectSession?.id,
...
function buildDisabledLiveDraftPayload(reason, message, details = {}) {
  return buildDisabledPayload(reason, message, {
    allyHovers: [],
...
module.exports = {
  CHAMP_SELECT_PHASE_BAN,
  CHAMP_SELECT_PHASE_PICK,
