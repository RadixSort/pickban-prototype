...
} = require("../public/summoner-spell-metadata.js");
 ...
 *
 * The parser keeps rune-element histograms, optional legacy page candidates,
 * summoner-spell set options, first-maxed skill options, grouped slot options,
 * ordered completed-item options, and only completed boots so later
 * aggregation can combine multiple enemy matchups without re-reading the raw
 * Qwik payload.
 */
    ...
    if (isStatModId(optionId)) {
      ...
      if (statEntry.games > 0) {
        addOptionRecord(statOptions, {
          ...
          slotIndexes: getStatModDefinition(optionId)?.slotIndexes || [],
...
function parseLolalyticsRenderedBuildPage(html = "", options = {}) {
  ...
  const buildLoader = {
    header: {
      ...
      lane:
        options.role ||
        qwikBuildData?.header?.lane ||
        qwikBuildData?.header?.defaultLane ||
        null,
      vsLane:
        options.enemyRole ||
        qwikBuildData?.header?.vsLane ||
        qwikBuildData?.header?.defaultVsLane ||
        null,
      n: totalGames,
    },
    runes: {
      stats: qwikBuildData?.runes?.stats || {},
    },
...
function buildLolalyticsRuneBuildLoader(payload, options = {}) {
  ...
  return {
    header: {
      ...
      vsLane: options.enemyRole || header.vsLane || header.defaultVsLane || null,
      n: totalGames,
    },
    runes: {
...
function extractRenderedQwikBuildData(html) {
  ...
  return {
    header:
      buildData.header && typeof buildData.header === "object" && !Array.isArray(buildData.header)
        ? buildData.header
        : {},
    ...
    runes: normalizeQwikRunes(buildData.runes),
    spells,
    startingItems,
    skillOrders,
    items: {
...
function normalizeQwikRunes(value) {
...
function createSummonerSpellSelection(spellId, spellNamesById) {
  return {
    ...
    name:
      ...
      getMetadataName(spellId, spellNamesById, `Spell ${spellId}`),
  };
}
