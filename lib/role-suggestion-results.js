...
function appendEnemyScores(
  ...
  for (const result of enemyResults) {
    ...
    fulfilledEnemyResults.push(result);
  }

  const weightedEnemyResults = expandEntriesByLaneOpponentWeight(fulfilledEnemyResults, {
    laneOpponentWeight,
    targetRole,
    getOpponentRole: getEnemyResultOpponentRole,
    getLaneOpponentLikelihood: (result) => result.value?.laneOpponentLikelihood,
    getFallbackScore: (result) => result.value?.rows?.size,
    getStableKey: (result) => result.value?.opponentChampionKey,
  });

  for (const result of weightedEnemyResults) {
    for (const [candidateKey, row] of result.value.rows) {
      const record = getCandidateRecord(candidateScores, candidateKey, championByKey);
...
function getEnemyResultOpponentRole(result) {
  if (result.value?.opponentRole) {
    return result.value.opponentRole;
  }

  for (const row of result.value?.rows?.values?.() || []) {
    if (row?.opponentRole) {
      return row.opponentRole;
    }
  }
