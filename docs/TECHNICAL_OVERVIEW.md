...
While Auto Import is active, the browser marks build requests as requiring complete matchups and starts them at Master+, independently of the selected rank filter. The server then tries each supported lower tier in order and accepts the first tier where every Counter Filter enemy returns data and the aggregate recommendation is usable. All enemies in one response use the same effective tier. If no tier completes the set, the response uses the tier with the most successful matchups (favoring the lower tier on ties). Manual build requests preserve the existing selected-tier partial-failure behavior, and the two modes use separate browser and server cache entries.
