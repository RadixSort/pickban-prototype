...
- `public/*.js`: focused browser helpers; shared rules are also loaded by Node tests and server code. `public/live-game-state.js` reconciles and retains trustworthy live inventory observations, while `public/live-game-poll.js` aligns full pulls to the in-game clock.
...
The Counter Filter keeps its selection through popup rerenders. An empty selection, final portrait removal, or clear action means all enemies. Filter changes request the same endpoint and cache by enemy subset; previous content and scroll position stay mounted while an uncached subset loads. Its horizontal/vertical orientation is a separate local-storage preference reused across popup closes, champions, and games. Vertical mode reserves a left rail and stacks the gold coin, enemy total, and ally total as three rows without the horizontal colon divider. Rune and item columns keep independent all-enemy/lane-only state.

Highest Win and Most Picked item paths each contain at most five ordered rows. Every row carries its primary option plus one distinct runner-up when available, rendered as `A or B`, so a path can expose up to ten unique items without adding rows or widening the popup. The selected ally's current completed-Legendary count greys the first N rows in both paths, capped at five.

Every build-popup request is marked as requiring complete matchups and starts at Master+, independently of Auto Import and the selected rank filter. The server then tries each supported lower tier in order and accepts the first tier where every Counter Filter enemy returns data and the aggregate recommendation is usable. All enemies in one response use the same effective tier. If no tier completes the set, the response uses the tier with the most successful matchups (favoring the lower tier on ties). Direct API requests that omit the completeness flag preserve the selected-tier partial-failure behavior, and the two request modes use separate browser and server cache entries.

The popup and toolbar share one Auto Import state. Before the selected ally owns a Legendary item, automatic filtering uses same-lane enemies for non-Junglers and all enemies for Jungle. Afterwards it uses enemies ranked 1–5 by global build gold. Selling every Legendary restores the pre-item rule. Empty automatic results fall back to all enemies, and each complete full live snapshot replaces manual filter state.
...
During `GameStart`, `InProgress`, and `Reconnect`, the server reads `/liveclientdata/playerlist`, `/liveclientdata/activeplayername`, and the optional `/liveclientdata/gamestats` clock from the local Game Client API. Those requests are unauthenticated and never reuse the League Client lockfile authorization header. The player identity is used only to split teams and is omitted from the response; held item rows are reduced to build-gold totals, rank, and Legendary completion before the browser response. Live champions and positions override speculative draft data. Normal Draft and Ranked require ten resolved participants split 5–5 before replacement; Practice Tool accepts its variable participant count. An incomplete first snapshot keeps metrics and automatic filtering unavailable. After a complete roster has been observed, same-session missing participants retain their last observed inventory metrics and the browser recomputes totals and ranks from the reconciled roster.

Each live snapshot resolves every held item ID against the patch-current authenticated League Client catalog and sums its full `priceTotal * count`, without excluding components, consumables, or zero-cost inventory rows. It then assigns ordinal global ranks in descending gold order; ties retain player-list order. A missing catalog cost marks that inventory observation unavailable instead of treating the item as free. Completed-Legendary progress is the current count of item stacks in the catalog-derived Legendary ID set, never a price threshold; first-item completion is derived from that count. The set prefers an explicit Legendary tier when available and otherwise derives eligible terminal recipe outputs, excluding boots, consumables, trinkets, unavailable placeholders, and unfinished support-quest transformations.

Live Client Data exposes no per-player fog visibility signal. For an enemy only, a missing inventory or a positive-to-zero observation therefore retains the last observed same-session inventory and Legendary state; later nonzero observations replace it. This prevents fog transitions from producing false zeroes without inferring unseen purchases. Allies always accept an observed zero so selling every Legendary item restores pre-item filtering. If the item catalog is temporarily unavailable, champion and lane updates continue while inventory metrics preserve their last known values and automatic filtering waits for accurate costs and classification.

After one complete live snapshot, the browser retains its last-known player item totals, global ranks, team totals, and Legendary progress when Live Client Data disconnects or the game ends. Those values remain displayable while the live-active flag is false. An enemy champion-composition change or a newly observed champion-select/live-game session invalidates the retained snapshot before it can be applied to the next game.

The browser polls independently of suggestion requests: every three seconds through champion select, incomplete live rosters, and the game-start handoff. After a complete roster snapshot, it reads `/liveclientdata/gamestats` and schedules each full pull near the next 10-second boundary of the in-game clock; elapsed request and recommendation time is included so work does not cause cumulative drift. If game time is unavailable, scheduling falls back to ten seconds from the prior full-poll start. Between full pulls, `statusOnly=1` checks gameflow every three seconds without reading the item catalog or Live Client Data, so game end and a subsequent champion select are detected promptly. Clicking Auto Import during a running game performs the first pull immediately. A `transition` response preserves Auto Import and last-known draft data while retrying; terminal phases and unsupported queues disable it. Session changes, phase exit, or disconnection invalidate ban state so late responses cannot reopen the panel, while completed live item metrics follow the separate retention rules above.
...
- The local Game Client API keeps its player-list, active-player-name, and game-stats resources compatible.
...
- `test/riot-live-draft.test.js`, `test/riot-live-game.test.js`, `test/live-game-state.test.js`, `test/live-game-poll.test.js`, and `test/riot-rune-import.test.js`: League draft normalization, full-cost inventory valuation and Legendary counts, retained live-rank invalidation, clock-aligned polling, and mutation.
- `test/lolalytics-build-parser.test.js` and `test/build-suggestion-results.test.js`: build-source regressions.
- `test/build-counter-filter.test.js`, `test/build-suggestion-view.test.js`, and `test/enemy-role-assignments.test.js`: popup subset and retained-gold visibility, A-or-B item rendering and progress greying, and probability-prioritized automatic roles.
